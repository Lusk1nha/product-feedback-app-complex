...
describe('RegisterUseCase', () => {
	...
	it('should create a user successfully', async () => {
		...
		mockUserRepository.findByUsername.mockResolvedValue(null) // Usuário não existe
	...
	it('should throw UserConflictError if username already exists', async () => {
		// Arrange
		const command = {
			username: faker.internet.username(),
			email: faker.internet.email(),
		...
		mockUserRepository.findByUsername.mockResolvedValue({ id: 1, ...command })

		// Act & Assert
		await expect(useCase.execute(command)).rejects.toThrow(UserConflictError)
